...
class TestEventDispatch(TestCase):
    ...
    def test_firing_event_notifies_all_subscribers(self, req_mock):
        ...
        new_subscriber = Subscription(webhook="http://ahmed.com", event=new_event)
        new_subscriber.save()

        new_subscriber = Subscription(webhook="http://wtf.com", event=new_event)
        new_subscriber.save()
    ...
    def test_firing_event_notifies_all_subscribers_different_events(self, req_mock):
        ...
        new_event2 = Event(event_id="BORED", retry_policy='{"max_retries": 10}')
        new_event2.save()

        new_subscriber2 = Subscription(webhook="http://wtf.com", event=new_event2)
