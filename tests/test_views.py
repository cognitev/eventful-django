...
class TestEventsEndpoint(TestCase):
    ...
    def test_gets_all_events(self):
        ...
        response = self.client.get('/eventful/events')
    ...
    def test_post_returns_error_405(self):
        reponse = self.client.post('/eventful/events')
...
class TestSubscriptionEndpoint(TestCase):
    ...
    def test_creates_subscription(self):
        Event.objects.create(event_id="HELLO", retry_policy='{"max_retries": 3}')
        response = self.client.post('/eventful/subscribe', {'event_id': 'HELLO', 'webhook': 'http://gamal.com'})
    ...
    def test_get_request_returns_405(self):
        reponse = self.client.get('/eventful/subscribe')
        self.assertEqual(reponse.status_code, 405)

    def test_error_404_when_not_valid_event(self):
        Event.objects.create(event_id="HELLO", retry_policy='{"max_retries": 3}')
        response = self.client.post('/eventful/subscribe', {'event_id': 'que?', 'webhook': 'http://gamal.com'})
    ...
    def test_error_400_missing_arguments(self):
        Event.objects.create(event_id="HELLO", retry_policy='{"max_retries": 3}')
        response = self.client.post('/eventful/subscribe', {'event_id': "HELLO", 'webhook': ''})
        response2 = self.client.post('/eventful/subscribe', {'event_id': "HELLO"})
        self.assertEqual(response.status_code, 400)
